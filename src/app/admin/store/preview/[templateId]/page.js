...
function BlueprintPreviewContent() {
  ...
  return (
    <div className={`relative min-h-screen ${isEmbed ? 'bg-white' : 'bg-gray-100'} flex flex-col`}>
        ...
        {/* Template Render */}
        <div className={`template-preview-frame ${viewport === 'mobile' && !isMobileDevice && !isEmbed ? 'py-6' : ''}`}>
            <div style={isEmbed ? { width: '100%' } : getViewportStyle()}>
                {HomeLayout ? (
                    <HomeLayout 
                        sections={sections}
                        theme={{}} 
